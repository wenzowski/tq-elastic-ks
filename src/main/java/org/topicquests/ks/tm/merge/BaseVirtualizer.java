...
public class BaseVirtualizer {
	...
	protected ITQDataProvider database;
	 ...
	 * @return
	 */
	protected boolean setUnionProperties(ISubjectProxy virtualNode, ISubjectProxy mergedNode) {
		...
		JSONObject sourceMap = mergedNode.getData();
		JSONObject virtMap = virtualNode.getData();
		...
		//update last edit
		if (result)
			virtMap.put(ITQCoreOntology.LAST_EDIT_DATE_PROPERTY, new Date());
		return result;
	}
	 ...
	 */
	IResult wireMerge(ISubjectProxy virtualProxy, ISubjectProxy targetProxy, Map<String, Double> mergeData,
	 ...
	 */
	private IResult relateNodes(ISubjectProxy virtualNode, ISubjectProxy targetNode,
	 ...
	 * @return
	 */
	void changePropertyValue(ISubjectProxy node, String key, String newValue) {
	...
	void addPropertyValue(ISubjectProxy node, String key, String newValue) {
	...
	IResult reWireNodeGraph(String mergedProxyLocator, String virtualProxyLocator, String mergeTupleLocator, ITicket credentials) {
		...
		List<ISubjectProxy>n = (List<ISubjectProxy>)xx.getResultObject();
		ITuple t;
		...
		if (xx.hasError())
			result.addErrorString(xx.getErrorString());
		n = (List<ISubjectProxy>)xx.getResultObject();
		if (n != null && !n.isEmpty()) {
			//time for surgery
			itr = n.iterator();
			while (itr.hasNext()) {
	...
	IResult performTupleSurgery(ITuple t, String newLocator, boolean isSubject) {
		String key = ITQCoreOntology.TUPLE_SUBJECT_PROPERTY;
		if (!isSubject)
			key = ITQCoreOntology.TUPLE_OBJECT_PROPERTY;
