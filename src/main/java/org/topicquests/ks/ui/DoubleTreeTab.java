...
public class DoubleTreeTab
  ...
  public void setEnvironment(SystemEnvironment env) {
	  environment = env;
	  database = environment.getDatabase();
	  model = database.getSubjectProxyModel();
	  populateLeftTree();
  }
  ...
  public void newButton_actionPerformed(ActionEvent e) {
	  ISubjectProxy n = model.newSubclassNode(selectedNodeLocator, "CHANGE ME", "CHANGE ME", "en", ITQCoreOntology.SYSTEM_USER, "CHANGE OR REMOVE ME", "CHANGE OR REMOVE ME", false);
	  editor.setTextToEdit(n.toJSONString());
  }

  public void editButton_actionPerformed(ActionEvent e) {
	  IResult r = database.getNode(selectedNodeLocator, credentials);
	  ISubjectProxy nn = (ISubjectProxy)r.getResultObject();
	  String xml = nn.toJSONString();
  ...
  public void newInstanceButton_actionPerformed(ActionEvent e) {
	  ISubjectProxy n = model.newInstanceNode(selectedNodeLocator, "CHANGE ME", "CHANGE ME", "en", ITQCoreOntology.SYSTEM_USER, "CHANGE OR REMOVE ME", "CHANGE OR REMOVE ME", false);
	  editor.setTextToEdit(n.toJSONString());
  }
